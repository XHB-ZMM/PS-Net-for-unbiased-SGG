...
from yacs.config import CfgNode as CN #xhb:这种包的导入方式是约定俗成的，需要创建CN()这个作为容器来装载我们的参数
#xhb:yacs就是一个为项目构建config文件的库，一个代码可读性强的项目，最好都有yacs
...
_C = CN() #xhb:一般都是这么写：_C = CN()

_C.MODEL = CN() #xhb:嵌套使用，MODEL就相当于一个容器了，可以装一下参数了
...
_C.MODEL.META_PREDICATE = "PredicatedRCNN"
...
# ---------------------------------------------------------------------------- #
# Backbone options xhb:主干特征网络的选取
# ---------------------------------------------------------------------------- #
...
# Overlap threshold for an RoI to be considered foreground (if >= FG_IOU_THRESHOLD) 与GT Box重叠大于0.5，就判断为前景
_C.MODEL.ROI_HEADS.FG_IOU_THRESHOLD = 0.5
# Overlap threshold for an RoI to be considered background
# (class = 0 if overlap in [0, BG_IOU_THRESHOLD)) 与GT Box重叠小于0.3，就判断为背景
...
'''
xhb:MODEL.ROI_RELATION_HEAD部分,我们需要在命令中经常切换参数的部分
'''
_C.MODEL.ROI_RELATION_HEAD = CN()
# share box feature extractor should be set False for neural-motifs
_C.MODEL.ROI_RELATION_HEAD.PREDICTOR = "MotifPredictor" #xhb:预测器
_C.MODEL.ROI_RELATION_HEAD.FEATURE_EXTRACTOR = "RelationFeatureExtractor" #xhb:特征提取器
_C.MODEL.ROI_RELATION_HEAD.POOLING_ALL_LEVELS = True
_C.MODEL.ROI_RELATION_HEAD.NUM_CLASSES = 51 #xhb:数据集中的类别数
_C.MODEL.ROI_RELATION_HEAD.BATCH_SIZE_PER_IMAGE = 64
_C.MODEL.ROI_RELATION_HEAD.POSITIVE_FRACTION = 0.25
_C.MODEL.ROI_RELATION_HEAD.USE_GT_BOX = True #xhb:是否采用GT—Box
_C.MODEL.ROI_RELATION_HEAD.USE_GT_OBJECT_LABEL = False #xhb:是否采用GT-LABEL
...
'''
xhb:MODEL.ROI_RELATION_HEAD.CAUSAL部分,我们需要在命令中经常切换参数的部分
'''
...
# Fusion
_C.MODEL.ROI_RELATION_HEAD.CAUSAL.FUSION_TYPE = 'sum' #xhb:融合方式 有sum和gate两种，训练和测试都要选
# causal context feature layer
_C.MODEL.ROI_RELATION_HEAD.CAUSAL.CONTEXT_LAYER = 'motifs' #xhb:预测器：有三种：motifs、vctree、vctranse，训练和测试都要选
...
_C.MODEL.ROI_RELATION_HEAD.CAUSAL.EFFECT_TYPE = 'none' # 'TDE', 'TIE', 'TE' #xhb:选择效应类型，测试的时候才选，训练时就是none

# proportion of predicates xhb:谓词的比例
...
_C.DTYPE = "float32" #xhb：但命令中通常是float16
