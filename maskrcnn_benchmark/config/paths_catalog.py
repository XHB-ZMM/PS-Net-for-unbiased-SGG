...
class DatasetCatalog(object):
    ...
    DATASETS = {
        ...
        "VG_stanford_filtered": {
            "img_dir": "/data/dataset/vg/VG_100K/",
    ...
    def get(name, cfg):
        if "coco" in name:
            ...
            #xhb：分离_train和前面的一串子名字
            p = name.rfind("_")
            name, split = name[:p], name[p+1:] #xhb:split = 'train'
            assert name in DatasetCatalog.DATASETS and split in {'train', 'val', 'test'}
            data_dir = DatasetCatalog.DATA_DIR #xhb:data_dir = 'datasets'
            args = copy.deepcopy(DatasetCatalog.DATASETS[name]) #xhb:args是一个字典了，
            for k, v in args.items(): #xhb：k='img_dir'   v='/data1/dataset/vg/VG_100K/'
