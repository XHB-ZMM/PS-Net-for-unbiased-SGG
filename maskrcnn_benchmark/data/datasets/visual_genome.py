...
class VGDataset(torch.utils.data.Dataset):
    ...
    def get_groundtruth(self, index, evaluation=False, flip_img=False):
        ...
        if self.filter_duplicate_rels:
            ...
            for (o0, o1, r) in relation: # sub0就是o0，obj0就是o1，relation里的是编号，准确的说是序号
        ...
        if evaluation:
            ...
            target.add_field("image_id", torch.as_tensor(self.img_info[index]['image_id']))  # 添加image_id 到target字典里
            return target
...
def get_VG_statistics(img_dir, roidb_file, dict_file, image_file, must_overlap=True):
    ...
    # f = open('/data/xuhongbo/xuhongbo.code/unbiased_sgg_xuhongbo_new/datasets/vg/Predicate_GT_test.csv', mode='a',
    #          encoding='utf-8', newline='')
    # csv_writer = csv.DictWriter(f, fieldnames=flieldnames)  # 列名
    ...
    for ex_ind in tqdm(range(len(train_data))):
        ...
        #
        #
        #         # with open("/data/xuhongbo/xuhongbo.code/unbiased_sgg_xuhongbo_new/datasets/vg/Predicate_GT.csv", 'a',newline='') as file_obj1:
        #         #     writer = csv.DictWriter(file_obj1, fieldnames=flieldnames)
        #         #     writer.writeheader()
        #         #     writer.writerows(predicate_gt)
