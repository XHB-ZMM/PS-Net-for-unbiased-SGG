...
# date     ：2022/1/13
# author   ：xhb
# describe ：CombinedROIHeads类的作用就是用来结合roi_box_head和roi_relation_head的,主要是调用build_roi_box_head和build_roi_relation_head函数
# output   ：建立起整个roi_heads，贡献函数build_roi_heads()供GeneralizedRCNN类调用
# ---------------------------------------------
class CombinedROIHeads(torch.nn.ModuleDict):
    ...
    def forward(self, features, proposals, targets=None, logger=None):
        losses = {} # 这个是一个batch里的loss，前面的add-loss是一张图像上的loss。我们的方法用不到add_loss
        ...
        if self.cfg.MODEL.RELATION_ON: #xhb：
            ...
            losses.update(loss_relation) #xhb:update函数用于添加字典里的key value对，使得字典扩大
            '''
            xhb:
            losses    : 是一个字典，里面有5个key了
            x         : torch.Size([32, 4096])
            detections: 类型为Boxlist，两张图像，每个都是16个框
            '''
