...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        if self.attribute_on:
            ...
            obj_dists, obj_preds, edge_ctx, obj_ctx, _ = self.context_layer(roi_features, proposals, logger)
...
class CausalAnalysisPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
        ...
        Returns:
            ...
            union_features (Tensor): (batch_num_rel, context_pooling_dim): visual union feature of each pair
        """
        num_rels = [r.shape[0] for r in rel_pair_idxs] #xhb：关系的数量 两张图像，每一张图像输出一个关系的数量
        num_objs = [len(b) for b in proposals] #xhb：目标的数量，也就是框的数量 两张图像，每一张图像有着不同的box数量 此刻是[16,7]，第一张图像16个框，第二张图像7个框
        obj_boxs = [get_box_info(p.bbox, need_norm=True, proposal=p) for p in proposals] #xhb：尺寸为[16,8]：16个框，每个框是8个参数 [7,8]同理

        assert len(num_rels) == len(num_objs)

        '''
        xhb:2022/1/12 训练两次，看看那些数值是变的
        :param post_ctx_rep:torch.Size([480, 4096])、torch.Size([282, 4096])
        :param pair_pred:torch.Size([480, 2]) 里面的数都是类别标签，一共480对、torch.Size([282, 2])，第二次是282对
        :param pair_bbox:torch.Size([480, 32])、torch.Size([282, 32])
        :param pair_obj_probs:torch.Size([32, 151])、torch.Size([282, 151, 2])
        :param binary_preds:None
        :param obj_dist_prob:torch.Size([32, 151])、torch.Size([23, 151])
        :param edge_rep:torch.Size([32, 2, 512])、torch.Size([23, 2, 512])
        :param obj_dist_list:两张图像，每个的尺寸都是torch.Size([16, 151])、两张图像，第一张是torch.Size([16, 151])，第二张是torch.Size([7, 151])
        '''
        ...
        # additional loss
        if self.training:
            rel_labels = cat(rel_labels, dim=0) #xhb：torch.Size([480])，但里面大多数是0，
